...
class Game:
    ...
    def parse_fen(fen: str, reverse_piece_placement: bool = True) -> 'Game':
        ...
        fen_info: FENInfo = GameEncoder.parse_fen(
            fen=fen,
            reverse_piece_placement=reverse_piece_placement
        )
        ...
        game = Game(
            ...
            en_passant_target=fen_info.en_passant_target,
        )
        game.moves_for_f_rule = fen_info.halfmove_clock
        ...
        for c in colors:
            if fen_info.castling_rights[c][RookSide.KING]:
                rook = game.board.get_square_or_piece(*squares[c.value][1])
                rook.rook_side = RookSide.KING

            if fen_info.castling_rights[c][RookSide.QUEEN]:
    ...
    def compute_game_state_hash(
        ...
        return GameEncoder.compute_game_state_hash(
            board=board,
